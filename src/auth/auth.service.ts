...
import { BadRequestException, HttpException, InternalServerErrorException } from '@exception';
...
export class AuthService {
  ...
  async signIn({ username, password }: SignInDto): ReturnPromiseWithErr<Tokens> {
    try {
      const [user, err] = await this.userService.findOne({ username: username }, true);
      if (err) throw err;
